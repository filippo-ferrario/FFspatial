...
spstFocal<-function(X, W_from=NULL, dist=NULL, side.cell=NULL, ...){
			...
			if(spatstat::is.owin(X) & is.null(W_from)) stop('"W_from" must be specified when X is an owin') 	
            if (!is.null(W_from)) {WW<- spatstat::as.owin(W_from)} else {WW<-W_from}
        	...
        	if (spatstat::is.owin(X)){
        				# 1) pixellate the owin object with the Transect window.
        				p2<-spatstat::pixellate.owin(X, W=WW, ..., eps=c(side.cell,side.cell))
        				# 2) assign the spatial domain of the transect
        				int<-spatstat::as.im(p2,W=WW)
        	...
        	if (spatstat::is.ppp(X))	{
        		...
        		if (!is.null(WW)) {spatstat::Window(X)<-WW} else {WW<-spatstat::Window(X)}
        		int<-spatstat::pixellate.ppp(X,eps=c(side.cell,side.cell),...) 
        		# 3) convert image to a rasterLayer
				ras<-raster::raster(int)
        		}	
			...
			mean_im<-spatstat::as.im(m, WW)
			im2<-spatstat::as.im(mean_im,WW)
			im2
}
