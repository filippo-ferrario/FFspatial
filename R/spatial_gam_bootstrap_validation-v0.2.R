...
#           The function extract the RMSE and AIC of each model runs, 
...
spt_gam_bts_val<-function( mod_fmla=NULL, Y_col=NULL, B=200,dataset=NULL,...)

{
  ...
  myGAM<-function(fmla=NULL, df_to_fit=NULL)
  	{ 
       mgcv::gam(formula = as.formula(fmla),
  ...
  M0<-lapply(mod_fmla, FUN=function(x) tryCatch(myGAM(fmla=x, df_to_fit),  error=function(err1) geterrmessage() ) )
  
  # identify which M0 objects are error messages
  m0id<-unlist(lapply(M0, function(x) is.character(x)))
  # store error messages
  mod_err<-M0[m0id]
  # filter M0 from error messages
  M0<-M0[!m0id]
  ...
  # attach error info as attribute
  attr(infoM0, 'errors')<-mod_err

  if (B==0){ 
     return(infoM0)
    } else{
      # remove models that failed on whole dataset from list
      mod_fmla_clean<-mod_fmla[!m0id] 
      ...
      for (b in 1:B){
           tryCatch({
                ...
                df_to_fit<- boot.dat
                # e$df_to_fit<-df_to_fit
                M1<-lapply(mod_fmla_clean, FUN=function(x) myGAM(fmla=x, df_to_fit) ) #use mod_fmla_clean to avoid bostrapping on models that failed
